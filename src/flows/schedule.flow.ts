...
const generateSchedulePrompt = (agendaSummary: string, history: string) => {
  const nowDate = getFullCurrentDate()
  return PROMPT_SCHEDULE
    .replace("{AGENDA_ACTUAL}", agendaSummary)
    .replace("{HISTORIAL_CONVERSACION}", history)
    .replace("{CURRENT_DAY}", nowDate)
}

/**
 * Maneja todo lo referente a agendar citas: revisa historial, comprueba huecos disponibles y propone fechas.
 */
const flowSchedule = addKeyword(EVENTS.ACTION).addAction(
  async (ctx, { extensions, state, flowDynamic }) => {
    ...
    const agenda = await getCurrentCalendar()
    const promptSchedule = generateSchedulePrompt(
      agenda?.length ? agenda : "ninguna",
      history,
    )
